...
if(step<5*max_len):    #append some frames of the terminal state such that they are visible in the animation
  for i in range(50):
    plt.plot( np.arange(-1.2,1.2,step=0.01),1-np.cos(np.arange(-1.2,1.2,step=0.01)), 'r')
    temp=[1.2]*10
    y_list=1-np.cos(temp)
...
anim=cam.animate(interval = 100, repeat = True, repeat_delay = 500)
anim.save('animate.mp4')
